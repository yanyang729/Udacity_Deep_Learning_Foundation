...
def test_build_rnn_modified(build_rnn):
    with tf.Graph().as_default():
        test_rnn_size = 256
        test_rnn_layer_size = 2

        test_cell = tf.contrib.rnn.MultiRNNCell([tf.contrib.rnn.BasicLSTMCell(test_rnn_size,state_is_tuple=True,reuse=tf.get_variable_scope().reuse)for _ in range(test_rnn_layer_size)], state_is_tuple=True)
...
def test_build_nn_modified(build_nn):
    with tf.Graph().as_default():
        test_input_data_shape = [128, 5]
        test_input_data = tf.placeholder(tf.int32, test_input_data_shape)
        test_rnn_size = 256
